<script lang="ts">
	...
	function isValid(): boolean {
		...
		if (cityLength == 0) {
			err = Error(
				"Nur eine Stadt darf angegeben werden. Für Städtebezeichnungen die aus mehreren Wörtern bestehen, bitte Bindestriche verwenden."
			);
			return false;
		}
		if (countryLength == 0) {
			err = Error(
				"Nur ein Land darf angegeben werden. Für Länderbezeichnungen die aus mehreren Wörtern bestehen, bitte Bindestriche verwenden."
			);
			return false;
		}
		if (blog.image == "") {
...
{#if succ}
	<section class="my-2 bg-green text-accent p-2 rounded-lg ">
		<h3>
			Dein Blog wurde kreiert. Besuche die Seite der <a class="underline-offset-1" href="/blog"
				>Blogbeiträge</a
